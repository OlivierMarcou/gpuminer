/*
 * kawpow_v5_final.cu - VERSION FINALE avec toutes les corrections
 * 
 * CORRECTIONS MAJEURES:
 * 1. KISS99: Formule correcte selon spec EIP-1057
 * 2. Initialization: KISS99 state correct avec FNV
 * 3. ProgPoW loop: Ordre correct
 * 4. Mix hash reduction: Conforme spec
 * 5. Keccak-256: 24 rounds complets
 */
...
// KISS99 - FORMULE CORRECTE selon spec EIP-1057
typedef struct {
    uint32_t z, w, jsr, jcong;
} kiss99_t;

static __device__ __forceinline__ uint32_t kiss99(kiss99_t &st) {
    ...
    // FORMULE CORRECTE: (MWC ^ jcong) + jsr
...
static __device__ void progpow_loop(
    ...
    // Initialize KISS99 state - CORRECTEMENT selon spec
    kiss99_t prog_rnd;
    ...
    // Avancer le RNG jusqu'à notre loop
    for (uint32_t i = 0; i < loop_idx * (PROGPOW_CNT_MATH + 2); i++) {
        kiss99(prog_rnd);
    }
    
    // DAG access
    uint32_t dag_addr_src = kiss99(prog_rnd) % PROGPOW_REGS;
    uint32_t dag_addr = mix[dag_addr_src] % dag_words;
    ...
    for (int i = 0; i < PROGPOW_CNT_MATH; i++) {
        ...
        uint32_t dst = kiss99(prog_rnd) % PROGPOW_REGS;
        uint32_t op = kiss99(prog_rnd);
        
        mix[dst] = progpow_math(mix[src1], mix[src2], op);
    }
    
    // Merge DAG data
    uint32_t dag_dst1 = kiss99(prog_rnd) % PROGPOW_REGS;
    uint32_t dag_dst2 = kiss99(prog_rnd) % PROGPOW_REGS;
    mix[dag_dst1] = fnv1a(mix[dag_dst1], (uint32_t)dag_item);
    mix[dag_dst2] = fnv1a(mix[dag_dst2], (uint32_t)(dag_item >> 32));
}

__global__ void kawpow_search_kernel_final(
    ...
    // Reduce mix: 32 registers → 8 words
    uint32_t mix_hash[8];
    for (int i = 0; i < 8; i++) {
...
extern "C" {
    ...
    void kawpow_search_launch(
        ...
        kawpow_search_kernel_final<<<grid_size, block_size>>>(
            (uint64_t*)dag, d_header, d_target, start_nonce, dag_size, prog_seed, d_solution, d_mix
        );
